...
public struct AggregateSnapshotPayload: Codable, Equatable {
  ...
  public struct Settings: Codable, Equatable {
    ...
    public init(
      ...
      lastSuccessfulSupabaseSync: Date? = nil,
  ...
  public init(from decoder: Decoder) throws {
    ...
    self.acknowledgedChangeIds = try container.decode([UUID].self, forKey: .acknowledgedChangeIds)
    self.chunk = try container.decodeIfPresent(ChunkMetadata.self, forKey: .chunk)
    ...
    self.schedules = try container.decode([Schedule].self, forKey: .schedules)
    // Default to empty array if key missing (backward compatibility with pre-history builds)
    self.history = try container.decodeIfPresent([HistorySummary].self, forKey: .history) ?? []
  }
  ...
  public func encode(to encoder: Encoder) throws {
    ...
    try container.encodeIfPresent(self.lastSyncedAt, forKey: .lastSyncedAt)
    try container.encode(self.acknowledgedChangeIds, forKey: .acknowledgedChangeIds)
    try container.encodeIfPresent(self.chunk, forKey: .chunk)
    try container.encodeIfPresent(self.settings, forKey: .settings)
    try container.encode(self.teams, forKey: .teams)
    try container.encode(self.venues, forKey: .venues)
...
public struct AggregateDeltaEnvelope: Codable, Equatable {
  ...
  public func decodePayload<T: Decodable>(
    as type: T.Type,
    using decoder: JSONDecoder = AggregateSyncCoding.makeDecoder()) throws -> T
  {
...
extension AggregateSyncCoding {
  ...
  fileprivate static var fallbackFormatter: ISO8601DateFormatter {
    let formatter = ISO8601DateFormatter()
