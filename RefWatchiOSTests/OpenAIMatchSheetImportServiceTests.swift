...
final class MatchSheetParseContractTests: XCTestCase {
  func testBuildPayload_whenMultipleImagesProvided_mapsToParserSchema() throws {
    ...
    let payload = MatchSheetParseContract.Testing.buildPayload(
      side: .home,
      expectedTeamName: "  Metro FC  ",
      images: images)
    let encoded = try MatchSheetParseContract.Testing.encodePayload(payload)
