...
final class SupabaseAuthController: ObservableObject {
  ...
  var statePublisher: AnyPublisher<AuthState, Never> {
    self.$state.removeDuplicates().eraseToAnyPublisher()
  }
  ...
  func signIn(email: String, password: String) async throws {
    do {
      try self.credentialValidator.validate(email: email, password: password)
    } catch {
  ...
  ///
  /// - Note: Supabase may require email confirmation. When no session is returned the
  ///   controller surfaces ``SupabaseAuthError/emailConfirmationRequired(email:)``.
  /// - Parameters:
  ...
  func signInWithApple(
    coordinator: SupabaseAppleSignInCoordinating? = nil) async throws
  {
    let client = try resolveClient()
    do {
  ...
  func signInWithGoogle(
    coordinator: SupabaseGoogleSignInCoordinating? = nil) async throws
  {
    let client = try resolveClient()
    do {
  ...
  private func observeAuthChanges() {
    ...
    self.authStateSubscription = Task {
  ...
  private func refreshState(using session: Session?) {
    if let session {
      ...
      self.state = .signedIn(userId: userId, email: email, displayName: displayName)

      self.profileSyncTask?.cancel()
      self.profileSyncTask = Task {
        do {
          try await self.profileSynchronizer.syncIfNeeded(session: session)
        } catch {
          AppLog.supabase.error("User profile sync failed: \(error.localizedDescription, privacy: .public)")
        }
        await self.clientProvider.refreshFunctionAuth()
      }
    } else {
