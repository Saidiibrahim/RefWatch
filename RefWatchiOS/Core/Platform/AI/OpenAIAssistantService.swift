...
final class OpenAIAssistantService: AssistantProviding {
  private enum RequestBuildError: LocalizedError {
    ...
    var errorDescription: String? {
      switch self {
      case .emptyConversation:
        "Cannot build OpenAI payload without at least one user message."
      }
    }
  }
  ...
  private enum ServiceError: LocalizedError {
    ...
    var errorDescription: String? {
      switch self {
      case .invalidResponse:
        "OpenAI request returned a non-HTTP response."
      case let .http(status, body):
        "OpenAI responded with HTTP \(status): \(body ?? "no body")"
      }
    }
  }
    ...
    AsyncStream { continuation in
      Task {
        ...
        do {
          ...
          let request = try Self.makeRequest(apiKey: self.apiKey, payload: payload)
...
extension OpenAIAssistantService {
  enum Testing {
    static func buildPayload(
      model: String,
      systemPrompt: String,
      messages: [ChatMessage]) throws -> ResponsesPayload
    {
    ...
    static func parseStream(
      lines: [String]) -> (chunks: [String], usage: ResponsesUsage?, shouldTerminate: Bool)
    {
...
extension OpenAIAssistantService {
    ...
    messages: [ChatMessage]) throws -> ResponsesPayload
  {
    ...
    #if DEBUG
    self.log("Finalizing payload — instructions length: \(systemPrompt.count), input count: \(inputMessages.count)")
    #endif
  ...
  static func makeRequest(apiKey: String, payload: ResponsesPayload) throws -> URLRequest {
    ...
    {
      self.log("Encoded payload: \(bodyString)")
    }
    #endif
    return request
  }
...
extension OpenAIAssistantService {
    ...
    continuation: AsyncStream<String>.Continuation) async throws
  {
    ...
    self.log("Received HTTP status \(httpResponse.statusCode)")

    guard (200...299).contains(httpResponse.statusCode) else {
      let body = await collectErrorBody(from: bytes)
      if let body {
        self.log("HTTP error body: \(body)")
      }
    ...
    if let usage = parser.usage {
      #if DEBUG
      debugPrint(
        "OpenAI usage — input: \(usage.inputTokens ?? 0), " +
  ...
  struct ResponsesStreamParser {
    ...
    mutating func handle(line rawLine: String, yield: (String) -> Void) {
      ...
      if let eventType = parseField("event", from: sanitizedLine) {
        if self.currentEvent != nil || !self.dataFragments.isEmpty {
          self.finalizeCurrentEvent(yield: yield)
        }
    ...
    mutating func finishIfNeeded(_ yield: (String) -> Void) {
      if !self.dataFragments.isEmpty || self.currentEvent != nil {
        self.finalizeCurrentEvent(yield: yield)
      }
    }
