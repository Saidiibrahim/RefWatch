...
final class ConnectivityClient: ConnectivityProviding {
  ...
  var isWatchAppInstalled: Bool {
    #if canImport(WatchConnectivity)
    return self.session?.isWatchAppInstalled ?? false
    #else
    return false
    #endif
  }
  ...
  func sendFixtureSummary(home: String, away: String, when: String) {
    ...
    guard self.isValid(team: home), self.isValid(team: away), self.isValid(when: when) else {
      #if canImport(OSLog)
      self.logger.warning("Rejected fixture payload due to validation failure")
      #endif
      return
    }
    ...
    guard session.isReachable else {
      #if canImport(OSLog)
      self.logger.info("WCSession not reachable; cannot send message")
      #endif
      return
    }
  ...
  private func isValid(team: String) -> Bool {
    ...
    return allowed.isSuperset(of: CharacterSet(charactersIn: trimmed))
  }
