...
extension TeamRecord {
  ...
  func markLocallyModified(at date: Date = Date(), ownerSupabaseId ownerId: String? = nil) {
    ...
    self.needsRemoteSync = true
    if let ownerId {
      self.ownerSupabaseId = ownerId
    }
  }
  ...
  func applyRemoteSyncMetadata(
    ...
    synchronizedAt syncDate: Date = Date())
  {
    self.ownerSupabaseId = ownerId ?? self.ownerSupabaseId
    self.remoteUpdatedAt = updatedAt
    self.needsRemoteSync = false
    self.lastModifiedAt = syncDate
  }
}
