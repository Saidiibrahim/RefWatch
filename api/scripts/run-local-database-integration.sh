...
umask 077
workspace=$(mktemp -d "${TMPDIR:-/tmp}/refwatch-greenfield-db.XXXXXX")
...
database_url="postgresql://postgres@127.0.0.1:${port}/${database_name}?sslmode=disable"
...
REFWATCH_LOCAL_DATABASE_URL="$database_url" \
  ./node_modules/.bin/vitest run --config vitest.local-database.config.ts
