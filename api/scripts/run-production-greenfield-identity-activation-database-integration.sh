...
umask 077
workspace=$(mktemp -d "${TMPDIR:-/tmp}/refwatch-identity-activation-db.XXXXXX")
...
REFWATCH_IDENTITY_ACTIVATION_LOCAL_DATABASE_URL="$database_url" \
  ./node_modules/.bin/vitest run \
    --config vitest.production-greenfield-identity-activation-database.config.ts
