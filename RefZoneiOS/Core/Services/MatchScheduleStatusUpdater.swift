...
final class MatchScheduleStatusUpdater: MatchScheduleStatusUpdating {
    ...
    func markScheduleInProgress(scheduledId: UUID) async throws {
        ...
        guard var schedule = schedules.first(where: { $0.id == scheduledId }) else {
            #if DEBUG
            AppLog.connectivity.debug("No schedule found for \(scheduledId.uuidString, privacy: .public) - skipping in-progress update")
            #endif
            return
        }
    ...
    func markScheduleCompleted(scheduledId: UUID) async throws {
        ...
        guard var schedule = schedules.first(where: { $0.id == scheduledId }) else {
            ...
            #if DEBUG
            AppLog.connectivity.debug("No schedule found for \(scheduledId.uuidString, privacy: .public) - likely manual match; skipping status update")
