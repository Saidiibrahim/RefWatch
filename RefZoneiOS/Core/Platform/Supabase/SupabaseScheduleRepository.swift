...
private extension SupabaseScheduleRepository {
  ...
  func performInitialSync() async {
    ...
    do {
      ...
      log.error("Initial schedule sync failed: \(error.localizedDescription, privacy: .public)")
    }
  }
...
private extension SupabaseScheduleRepository {
  ...
  func performRemoteDeletion(id: UUID) async {
    do {
      try await api.deleteScheduledMatch(id: id)
  ...
  func pushDirtyMatches() async throws {
    ...
    let dirty = all.filter { $0.needsRemoteSync }
    guard dirty.isEmpty == false else { return }
    for match in dirty {
      pendingPushes.insert(match.id)
  ...
  func pullRemoteUpdates(for ownerUUID: UUID) async throws {
    let remote = try await api.fetchScheduledMatches(ownerId: ownerUUID, updatedAfter: remoteCursor)
    guard remote.isEmpty == false else { return }
    let filtered = remote.filter { !pendingDeletions.contains($0.id) }
...
private extension SupabaseScheduleRepository {
  func mergeRemoteMatches(_ remoteMatches: [SupabaseScheduleAPI.RemoteScheduledMatch], ownerUUID: UUID) throws {
    var didChange = false
    for remote in remoteMatches {
      if let record = try store.record(id: remote.id) {
        ...
        apply(remote: remote, to: record, ownerUUID: ownerUUID)
        didChange = true
      } else {
        try insertRemoteMatch(remote, ownerUUID: ownerUUID)
        didChange = true
      }
    }
    if didChange {
  ...
  func insertRemoteMatch(_ remote: SupabaseScheduleAPI.RemoteScheduledMatch, ownerUUID: UUID) throws {
  ...
  func apply(remote: SupabaseScheduleAPI.RemoteScheduledMatch, to record: ScheduledMatchRecord, ownerUUID: UUID) {
  ...
  func makeUpsertRequest(for record: ScheduledMatchRecord, ownerUUID: UUID) -> SupabaseScheduleAPI.UpsertRequest {
