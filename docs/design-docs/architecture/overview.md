...
The architecture is in migration, not production cutover. As of 2026-07-17, the Worker workspace and active iOS composition/repositories route through Clerk and the backend API—including authenticated reference-catalog reads—and the app builds without Supabase configuration. The Supabase SDK/package dependency is removed. Staging connectivity passed, and the production foundation now includes all 16 migrations, the exact 5/54 seed, restricted roles, cache-disabled Hyperdrive, inactive ledger resources, and an unrouted write-disabled Worker coordinated with `refwatch.ibby.ai`. Final identity/data import, remaining Clerk setup, provider-routed authenticated acceptance, rollback completion, physical devices, traffic/writes, and legacy Supabase-named repository/type/source-path cleanup remain separately gated. Supabase source and migration files are legacy evidence until cutover validation and rollback gates close.
