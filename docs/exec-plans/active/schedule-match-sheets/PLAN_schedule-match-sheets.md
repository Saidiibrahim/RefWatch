...
- [x] PM follow-up on 2026-03-31 restored optional Team Library name autofill in `UpcomingMatchEditorView`, but kept scheduled-match editing free-text and schedule-owned with no `TeamRecord` binding, reference-team materialization, or provenance rewrite.
...
- Follow-up product direction removed the remaining iPhone editor dependence on live `TeamRecord` selection; the editor must now author schedule-owned manual/ad hoc sheets while preserving any `sourceTeamId` / `sourceTeamName` already stored on imported or older sheets. A later PM follow-up restored Team Library name autofill only, using already-saved library teams without reintroducing `TeamRecord`-driven sheet state or reference-team materialization.
...
- Validation evidence moved after the follow-up pass:
  ...
  - the targeted iPhone regression slice now passes on `iPhone 15 Pro Max (iOS 17.0.1)`, including `SwiftDataScheduleStoreTests`, `MatchSheetImportViewModelTests`, and `OpenAIMatchSheetImportServiceTests`
  ...
  - the post-fix simulator build now passes on the same `iPhone 15 Pro Max (iOS 17.0.1)` destination
...
- Date/Author: 2026-03-25 / Codex
- Decision: iPhone scheduled-match authoring no longer chooses or reseeds from `TeamRecord`; it edits free-text schedule-owned sheets, may autofill the visible name from already-saved Team Library teams only, and only preserves `sourceTeamId` / `sourceTeamName` already present on the sheet.
...
- Validation now shows targeted shared-core tests passing, the targeted iPhone regression slice passing, decoupling-specific save-path coverage added, and the post-fix iPhone simulator build passing on `iPhone 15 Pro Max (iOS 17.0.1)`.
